define('singleSection',['jquery','TweenMax','preloadAssets','scrollmagicmanager','cjsLoader','utils'],
	...
	proto.preLoadSelf = function(dtd){
		...
		$.when(preload.preloadImgs(tmpSelf.$dom)).done(function(){
			...
			$.when(tmpSelf.requestAddSMM(),tmpSelf.checkModules()).done(function(){
				...
				dtd.resolve();
				if(tmpSelf.$dom[0].classList[1] === 'content-head') {
					additionalFunc.apply(tmpSelf);
				} else if (tmpSelf.$dom[0].classList[1] === 'content-virtual-tour-intro') {
					initVirtualTourBeforeCta.apply(tmpSelf);
				}
			});
		});
	};
	...
	**/
	...
	function additionalFunc() {
		var tmpThis = this;
		var arrowAni = $('#intro-head-arrow');
		var nextSection = $("html, body").find('.content-open');

		arrowAni.on('click',function(){
			$("html, body").animate({ scrollTop: nextSection.offset().top-117 });
			window.G.track("Scroll CTA", "Scroll CTA Click");
		});

		var afterTourAni = $('#after-virtual-tour-arrow');

		afterTourAni.on("click", function() {
			console.log("Scrolling Past Virtual Tour");
			$("html, body").animate({ scrollTop: $("#legends-section").offset().top-118 });
			window.G.track("Scroll CTA", "Scroll Past Virtual Tour");
		});
	}

	function initVirtualTourBeforeCta() {
		var beforeTourAni = this.$dom.find('#before-virtual-tour-arrow');

		beforeTourAni.on("click", function() {
			$("html, body").animate({ scrollTop: $("#virtual-tour-container").offset().top-118 });
			window.G.track("Scroll CTA", "Scroll to Virtual Tour");
		});
	}
